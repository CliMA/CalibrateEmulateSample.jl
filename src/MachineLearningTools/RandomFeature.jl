...
"""
Abstract supertype for covariance structure parameterisations used in random-feature
kernels. Concrete subtypes — [`OneDimFactor`](@ref), [`DiagonalFactor`](@ref),
[`CholeskyFactor`](@ref), [`LowRankFactor`](@ref), [`HierarchicalLowRankFactor`](@ref)
— control the number of hyperparameters and the shape of the feature distribution.
"""
...
Return the nugget regularisation term `eps` added to the diagonal of the covariance matrix
...
Return the input-space `CovarianceStructureType` stored in `kernel_structure`.
"""
...
Return the output-space `CovarianceStructureType` stored in `kernel_structure`.
"""
...
Return the joint-space `CovarianceStructureType` stored in `kernel_structure`.
"""
...
calculate_n_hyperparameters(d::Int, lrf::LowRankFactor) = Int(d * rank(lrf) + d)
...
function hyperparameters_from_flat(x::V, lrf::LowRankFactor) where {V <: AbstractVector}
    ...
    r = rank(lrf)
    d = Int(length(x) / (r + 1)) # l = d + d*r = (r+1)*d
...
function build_default_prior(name::SS, n_hp::Int, df::DiagonalFactor) where {SS <: AbstractString}
    return ParameterDistribution(
        Dict(
            "name" => "$(name)_diagonal",
            "distribution" => VectorOfParameterized(repeat([Normal(log(sqrt(0.1 / n_hp)), 2)], n_hp)),
            "constraint" => repeat([bounded_below(1e-8 / n_hp)], n_hp),
        ),
    )
end

function build_default_prior(name::SS, n_hp::Int, df::CholeskyFactor) where {SS <: AbstractString}
    return constrained_gaussian("$(name)_cholesky", 0.0, 0.1 / n_hp, -Inf, Inf, repeats = n_hp)
end

function build_default_prior(name::SS, n_hp::Int, lrf::LowRankFactor) where {SS <: AbstractString}
    r = rank(lrf)
    d = Int(n_hp / (r + 1)) # n_hp = d + d*r
