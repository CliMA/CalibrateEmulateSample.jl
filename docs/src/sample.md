...
AbstractMCMC isn't needed. We implement three variants of MH with three different Samplers: `RWMetropolisHastings`,
`pCNMetropolisHastings`, and `BarkerMetropolisHastings`, all of which are subtypes of `AdvancedMH.MHSampler`. The
constructor for all Samplers is [`MetropolisHastingsSampler`](@ref); the different Samplers are specified by passing a
[`MCMCProtocol`](@ref) object to this constructor.

Each concrete `MHSampler` stores a `cholesky_L` field — the lower Cholesky factor of the (encoded) prior covariance,
shaping the proposal noise (`pCNMetropolisHastings` additionally stores `prior_mean`, since pCN's proposal contracts
toward the encoded prior mean rather than toward `0`). A new proposal is generated by
...
- [`BarkerSampling`](@ref) for the (gradient-based) Barker proposal, see the protocol summary above.

!!! note "Every sampler computes its own Hastings correction"
    pCN's and Barker's proposals are *not* symmetric, so the acceptance ratio must include a Hastings
    correction `log q(θ_from|θ_to) − log q(θ_to|θ_from)`. Each sampler implements this via two required
    extension points: `log_transition_density` (the forward transition log-density its `propose` draws
    from) and `AdvancedMH.logratio_proposal_density` (which combines the forward and reverse evaluations
    into the correction term). There is deliberately no generic symmetric fallback: an earlier version of
    this package silently assumed all proposals were symmetric, which caused the pCN and Barker samplers
    to target the wrong distribution.
