...
We will show how Calibrate-Emulate-Sample can be used to learn the parameters of
the initial cloud droplet mass distribution from observations of the moments of
that mass distribution at a later time. 
...
the true parameter values---in more realistic applications, one would use actual measurements of cloud properties.
...
Calibrate-Emulate-Sample to learn model parameters.
...
that runs Cloudy with a given set of parameters,
...
which at time ``t = 0`` has parameters ``\phi_0 = [N_{0, 0}, k_0, \theta_0]``. We will then try to learn these parameters from observations ``y = [M_0(t_{end}),
M_1(t_{end}), M_2(t_{end})]`` of the zeroth-, first-, and second-order moments
...
We use [`constrained_gaussian`](@ref constrained-gaussian) to add the desired scale and bounds to the prior distribution, in particular we place lower bounds to preserve positivity (and numerical stability). 
...
perform the MCMC sampling. We also need some functionality of
...
The last step is to plug the emulator into a Markov chain Monte Carlo (MCMC)
algorithm, which is then used to produce samples from the posterior distribution
of the parameters. Essentially, the emulator acts as a stand-in for the
original forward model (which in most cases of interest is computationally
expensive to run) during the MCMC sampling process.
...
the apex of the posterior distribution should converge near the true values,
indicating a high-quality estimation Additionally, visualizing the prior
distributions alongside the posteriors shows the distributional effected by the Bayesian learning process.
