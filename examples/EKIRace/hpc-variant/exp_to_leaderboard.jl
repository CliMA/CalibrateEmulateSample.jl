...
for (N_ens, rng_idx) in valid_file_items
    ...
    for k in k_values
        ...
        if has_forcing
            ...
            for (qi, qp) in enumerate(marginal_coverage_quantiles)
                forcing_coverage_arr[i, j, k, qi] =
                    mean(truth_forcing_vec .<= [quantile(fs[:, d], qp) for d in 1:n_forcing])
            end
        end
    end
end
...
if has_forcing
    truth_forcing_v =
        defVar(ds, "truth_forcing", Float64, ("random_seed", "ensemble_size", "forcing_dim"); fillvalue = NaN)
...
output_budget_v = defVar(
    ds,
    "output_budget_to_target",
    Float64,
    ("random_seed", "ensemble_size", "target_scaling", "coverage_quantile");
    fillvalue = NaN,
)
...
if has_forcing
    ...
    forcing_mahal_v =
        defVar(ds, "forcing_mahalanobis", Float64, ("random_seed", "ensemble_size", "k_iter"); fillvalue = NaN)
    ...
    forcing_logpdf_true_v_map_v =
        defVar(ds, "forcing_logpdf_true_v_map", Float64, ("random_seed", "ensemble_size", "k_iter"); fillvalue = NaN)
    ...
    forcing_plr_top_v =
        defVar(ds, "forcing_plr_mahalanobis_top", Float64, ("random_seed", "ensemble_size", "k_iter"); fillvalue = NaN)
    ...
    forcing_plr_res_v = defVar(
        ds,
        "forcing_plr_mahalanobis_residual",
        Float64,
