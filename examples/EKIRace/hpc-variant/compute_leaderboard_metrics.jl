...
ens_vals = try
    Int.(ncd["ensemble_size"][:])
catch
    collect(1:n_ens_size)
end
kiter_vals = try
    Int.(ncd["k_iter"][:])
catch
...
(
    haskey(ncd, "forcing_mahalanobis") ||
    haskey(ncd, "forcing_plr_mahalanobis_top") ||
    haskey(ncd, "forcing_coverage") ||
    (haskey(ncd, "forcing_samples") && haskey(ncd, "truth_forcing"))
) && push!(avail_spaces, :forcing)
(
    haskey(ncd, "output_mahalanobis") ||
    haskey(ncd, "output_plr_mahalanobis_top") ||
    haskey(ncd, "output_coverage") ||
    (haskey(ncd, "output_samples") && !isnothing(y_truth))
) && push!(avail_spaces, :output)
(haskey(ncd, "forcing_plr_mahalanobis_top") || haskey(ncd, "output_plr_mahalanobis_top")) &&
    push!(avail_metrics, :plr_mahalanobis)
(
    (haskey(ncd, "forcing_samples") && haskey(ncd, "truth_forcing")) ||
    (haskey(ncd, "output_samples") && !isnothing(y_truth)) ||
    haskey(ncd, "forcing_coverage") ||
    haskey(ncd, "output_coverage")
) && push!(avail_metrics, :coverage)
...
isnothing(avail_quantiles) || println(
    "  calibration_check_quantiles = $(calibration_check_quantiles)  (used to select quantile subset for budget metric)",
)
println("="^72)
...
if show_space(:param) && (show_metric(:mahalanobis) || show_metric(:logratio))
    ...
    for (qi, p) in enumerate(calibration_check_quantiles),
        (ei, ev) in enumerate(ens_vals),
        (ki, kv) in enumerate(kiter_vals)
...
if show_space(:forcing) &&
   (show_metric(:mahalanobis) || show_metric(:logratio)) &&
   haskey(ncd, "forcing_mahalanobis") &&
   haskey(ncd, "forcing_logpdf_true_v_map")
    ...
    for (qi, p) in enumerate(calibration_check_quantiles),
        ...
        push!(
            ...
            (
                ...
                ens_size = ev,
                k_iter = kv,
                mh_score = fmh_scores[qi, ei, ki],
                mh_score_std = fmh_scores_std[qi, ei, ki],
                lp_score = flp_scores[qi, ei, ki],
                lp_score_std = flp_scores_std[qi, ei, ki],
                mh_coverage = fmh_coverage[ei, ki],
                lp_coverage = flp_coverage[ei, ki],
            ),
        )
    end
...
if show_space(:output) &&
   (show_metric(:mahalanobis) || show_metric(:logratio)) &&
   haskey(ncd, "output_mahalanobis") &&
   haskey(ncd, "output_logpdf_true_v_map")
    ...
    for (qi, p) in enumerate(calibration_check_quantiles),
        ...
        push!(
            ...
            (
                ...
                ens_size = ev,
                k_iter = kv,
                mh_score = omh_scores[qi, ei, ki],
                mh_score_std = omh_scores_std[qi, ei, ki],
                lp_score = olp_scores[qi, ei, ki],
                lp_score_std = olp_scores_std[qi, ei, ki],
                mh_coverage = omh_coverage[ei, ki],
                lp_coverage = olp_coverage[ei, ki],
            ),
        )
    end
...
if show_metric(:plr_mahalanobis)
    for (space_label, space_sym, top_sym, res_sym, n_dim_key) in [
        ...
        println("\n" * "="^72)
        println(
            "$(space_label)-space perturbed-low-rank Mahalanobis  (k=$(n_lowrank_modes) modes, n=$(n_dim) total dims)",
        )
        ...
        for (part_label, arr, ref_dof) in [
            ...
            scores_std =
                Array{Union{Float64, Missing}}(missing, length(calibration_check_quantiles), n_ens_size, n_k_iter)
            ...
            for (qi, p) in enumerate(calibration_check_quantiles),
                ...
                push!(
                    ...
                    (
                        ...
                        ens_size = ev,
                        k_iter = kv,
                        score = scores[qi, ei, ki],
                        score_std = scores_std[qi, ei, ki],
                        coverage = coverage[ei, ki],
                    ),
                )
            end
            ...
            for ens in filter(show_ens, sort(unique(df_plr.ens_size)))
                ...
                for (qi, p) in enumerate(calibration_check_quantiles)
                    ...
                    println(
                        "    q = $(p)  [chi-sq($(ref_dof)) bound = $(round(qq_ref[qi], digits=2)),  expected score ≈ $(p)]",
                    )
...
if show_metric(:coverage)
    ...
    if show_space(:output) && haskey(ncd, "output_samples") && !isnothing(y_truth)
        ...
        actual_out_dim != n_out &&
            @warn "output_samples trailing dim ($actual_out_dim) ≠ output_dim ($n_out); raw coverage uses actual dim, R-whitened coverage disabled"
        ...
        for (label, do_white) in [("raw", false), ("R-whitened PCA", true)]
            ...
            if do_white
                println(
                    "  x̃_d = (Vᵀx)_d / √λ_d  (R = VΛVᵀ, top $(k_R)/$(n_out) modes, $(round(Int, 100*R_variance_retain))% var threshold)",
                )
...
if show_metric(:budget_for_coverage)
        ...
        for (ei, ev) in enumerate(ens_vals)
            ...
            for (si, c) in enumerate(budget_target_scalings)
                ...
                if n_conv == 0
                    println(
                        @sprintf(
                            "  %-10d  %-6.2f  %8.1f  %8.1f  [%5.0f,%5.0f]  %8.1f  %8.1f  %d/%d",
                            ev,
                            c,
                            0.0,
                            0.0,
    ...
    function _bfcov_make_figure(
        ...
        for (si, c) in enumerate(budget_target_scalings)
            ...
            for (xi, (ei, ev)) in enumerate(filt_ens)
                ...
                annotate!(p, [xi], [ann_y], [Plots.text("$(n_conv)/$(n_tot)", 7, :center, :black)]; subplot = si)
            end
        ...
        for (si, c) in enumerate(budget_target_scalings)
            ...
            frac_fail_vec = [(n_rng - count(!isnan, budgets_mat[:, ei, si])) / n_rng for (ei, _ev) in filt_ens]
    ...
    for (space_label, space_sym, cov_key, dim_key, budget_nc_key, iters_nc_key) in [
        (
            ...
            "output_coverage",
            "output_dim",
            "output_budget_to_target",
            "output_iters_to_target",
        ),
        (
            "Forcing-space raw",
            :forcing,
            "forcing_coverage",
            "forcing_dim",
            "forcing_budget_to_target",
            "forcing_iters_to_target",
        ),
    ]
