...
if !haskey(first_loaded, "pushforward_output_samples")
    error("Pushforward data not found in $(first_post_fn). Run pushforward_from_posterior_l63.jl first.")
end
...
for (N_ens, rng_idx) in valid_file_items
    ...
    if !haskey(loaded, "pushforward_output_samples")
        @warn "Pushforward data missing for $(post_fn); skipping. Run pushforward_from_posterior_l63.jl first."
        continue
    end
