...
n_samples_pushforward = 400 # num. samples to pushforward through lorenz
...
for (N_ens, rng_idx) in valid_file_items
    ...
    if !haskey(loaded_p, "pushforward_output_samples")
        error("Pushforward data not found in $(post_fn). Run pushforward_from_posterior_l96.jl first.")
    end
    ...
    elseif force_case == "flux-force"
        ...
        (truth_params_constrained, truth_params_obj.model, truth_params_obj.sample_range)
    end
    ...
    prior_forcings = hcat(
        [
            forcing(build_forcing(truth_params_obj, prior_samples_constrained[:, j], structure, sample_range), x0)
    ...
    for k in k_values
        ...
        if is_const
            ...
            plot!(
                pb,
                xaxis_forcing,
                prior_forcing_quantiles[:, 2],
                color = :grey,
                label = "prior",
                linewidth = 4,
                ribbon = [prior_forcing_quantiles[:, 2] - prior_forcing_quantiles[:, 1] prior_forcing_quantiles[:, 3] -
                                                                                        prior_forcing_quantiles[:, 2]],
                fillalpha = 0.2,
            )
            plot!(
                pb,
                xaxis_forcing,
                post_forcing_quantiles[:, 2],
                color = :blue,
                label = "posterior",
                linewidth = 4,
                ribbon = [post_forcing_quantiles[:, 2] - post_forcing_quantiles[:, 1] post_forcing_quantiles[:, 3] -
                                                                                      post_forcing_quantiles[:, 2]],
                fillalpha = 0.2,
            )
        end
