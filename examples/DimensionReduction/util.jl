...
function do_mcmc(
    ...
    sampler = if mcmc_sampler == :mala
        MALA(x -> MvNormal(0.0001 * prior_cov * x, 0.0001 * 2 * prior_cov))
    elseif mcmc_sampler == :rw
        RWMH(MvNormal(zeros(dim), 0.01prior_cov))
    else
    ...
    for batch in 1:num_batches
        ...
        chain = sample(
            ...
            num_chains_in_batch;
            chain_type = Chains,
            initial_params = [initial_guess for _ in 1:num_chains_in_batch],
        )
        samp = vcat(
            [vec(MCMCChains.get(chain, Symbol("param_$i"))[1]'[:, (end ÷ 2):subsample_rate:end])' for i in 1:dim]...,
        )
