...
function main()
    ...
    for (idx, rv_rinfo) in enumerate(rvs_rinfos)
        ...
        encoder_schedules_li = [
            [(decorrelate_structure_mat(), "in_and_out"), (likelihood_informed(retain_info = rinfo, iters = 1:i), "in")] for i in mask
                ]
    ...
    # build dataframe
    pairs =[(reduced_dims[i, j+1], all_errs[i, j])
            for i in axes(all_errs, 1), j in axes(all_errs, 2)]
