...
# emulate. This step is not needed for practical applications.
...
# Get training points: Use ensemble at last step (EKS) or all stages (EKI)
...
# other parameters to check GP landscape
...
# u0 has been modified by MCMCObj in the test, so we reset it to the EKI ensemble.
