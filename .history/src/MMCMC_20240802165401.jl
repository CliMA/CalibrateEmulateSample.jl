...
function EmulatorPosteriorModel(
    ...
    return AdvancedMH.DensityModel(
        function (θ)
        try ##########
            @info "Starting EmulatorPosteriorModel calculation"
            @info "Parameters (θ): $θ"
            ...
            @info "Emulator prediction: g=$g, g_cov=$g_cov"
            ...
            @error "Error in EmulatorPosteriorModel" exception=(e, catch_backtrace())
            rethrow(e)
        end#########
        end,
    )
...
function AdvancedMH.propose(
    ...
    try
        @info "Starting MALA proposal step"
        @info "Current state parameters: $(current_state.params)"
        ...
        @info "Computed gradient: $log_gradient"

        proposed_state = current_state.params .+ (stepsize / 2) .* log_gradient .+ sqrt(stepsize) * rand(rng, sampler.proposal)

        @info "Proposed state: $proposed_state"
        return proposed_state
    catch e
        @error "Error in MALA propose step" exception=(e, catch_backtrace())
        rethrow(e)
    end
end
